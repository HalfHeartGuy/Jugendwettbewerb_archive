...
print("createARandom3x3SudukoBox:", createARandom3x3SudukoBox())
print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx 2023-05-27#####################")
print("sudoko", sudoko)


import math
def findAllNumberInSameColumnRowBox(indexM:int, indexN:int):
    numberInSameRow = [zahl for zahl in sudoko[indexM] if zahl != 0]
    print("numberInSameRow", numberInSameRow)
    numberInSameColumn = [sudoko[i][indexN] for i in range(0, 9) if sudoko[i][indexN] != 0]
    print("numberInSameColumn", numberInSameColumn)
    numberInSameBox = [sudoko[math.floor(indexM/3) * 3 + i][math.floor(indexN/3)*3+j] for i, j in range(0,3) if sudoko[math.floor(indexM/3) * 3 + i][math.floor(indexN/3)*3+j] != 0]
    print("numberInSameBox", numberInSameBox)

    return numberInSameRow + numberInSameColumn + numberInSameBox

findAllNumberInSameColumnRowBox(3,4)

## diese funktion gibt eine Randamzahl zurück, die NICHT in dem Ergebnis von findAllNumberInSameColumnRowBox ist.
def fillOneCell(indexM,indexN):
    return 0
